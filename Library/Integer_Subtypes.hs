...
module Integer_Subtypes (
   ...
   from_Pos_to_Nat,       -- :: Pos -> Nat
	...
	instance Random Natural where
	   random gen = let (x, gen') = randomR (0, toInteger (maxBound :: Int)) gen in
	      (Natural x, gen')
	   randomR (Natural from, Natural to) gen = let (x, gen') = randomR (from, to) gen in
	      (Natural x, gen')
	...
	instance Random Positive where
	   random gen = let (x, gen') = randomR (1, toInteger (maxBound :: Int)) gen in
	      (Positive x, gen')
	   randomR (Positive from, Positive to) gen = let (x, gen') = randomR (from, to) gen in
	      (Positive x, gen')
	...
	instance Random Int_Range where
	   random gen = let (x, gen') = random gen in
	      (Int_Range x, gen')
	   randomR (Int_Range from, Int_Range to) gen = let (x, gen') = randomR (from, to) gen in
	      (Int_Range x, gen')
	...
	instance Random Nat where
	   random gen = let (x, gen') = randomR (0, maxBound :: Int) gen in
	      (Nat x, gen')
	   randomR (Nat from, Nat to) gen = let (x, gen') = randomR (from, to) gen in
	      (Nat x, gen')
	...
	instance Random Pos where
	   random gen = let (x, gen') = randomR (1, maxBound :: Int) gen in
	      (Pos x, gen')
	   randomR (Pos from, Pos to) gen = let (x, gen') = randomR (from, to) gen in
	      (Pos x, gen')
